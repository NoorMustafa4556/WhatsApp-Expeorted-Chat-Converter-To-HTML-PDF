...
def detect_date_format(timestamps):
    formats = [
        ...
        '%m/%d/%y, %I:%M%p',  # Android 12-hour
        '%d/%m/%y, %I:%M%p',  # Android 12-hour
        '%m/%d/%y, %I:%M %p',  # Android 12-hour with space
        '%d/%m/%y, %I:%M %p',  # Android 12-hour with space
...
def create_media_embed(message, subfolder, senderclass):
    ...
    if '(file attached)' in message or '<attached:' in message:  # for both Android and iOS
        ...
        file_path = html.escape(f"{subfolder}/{file_name}")
...
@media print {
    ...
    .left, .right, .bubble, .date, img, video { 
...
'''
    ...
    for timestamp, sender, message, timestamp_str in messages:
        ...
        message = create_media_embed(message, folder_name, senderclass)
        message_id += 1
